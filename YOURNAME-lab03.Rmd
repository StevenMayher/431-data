...
## Read in `chr_data.csv` for Questions 6-10

You'll need to do this on your own. Use the ideas from Lab 02 and the instructions from Labs 02 and 03 to help you. We'll assume in what follows that you've called the resulting R object `chr_data`.

```{r}
chr_data <- read_csv("https://raw.githubusercontent.com/THOMASELOVE/431-data/main/chr_data.csv")
```

Make sure the result contains 3142 rows and 91 columns.
...
To get started here, we need to pull a sample from the `chr_data` we imported earlier. Let's call that `lab03_counties` as an R object. We'll get started with ...

```{r}
set.seed(2012022) # following the instructions

lab03_counties <- sample_n(chr_data, size = 750)
```
